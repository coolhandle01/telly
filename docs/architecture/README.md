...
broadcast day testable with no network, no browser and no real clock.
...
| [player.md](player.md) | The YouTube IFrame API, the card-as-base-signal rule, and the failure modes it has to survive. |
| [tokens.md](tokens.md) | Google Identity Services, the user-gesture rule, and why the token is held in the tab's session storage. |
...
src/
  ...
  testcard/    the five rotating card designs and the fault card, their
               geometry, and the renderer
  audio/       what comes out of the speaker: the line-up tone, the hiss, and
               the cabinet's clicks
  ui/          the screen, the cabinet it sits in, and faultMessage, where a
               failed load, sign-in or sign-out becomes words
...
| A programme may go out twice a day, four hours apart, with the second showing scored at a fifth | [scheduling.md](scheduling.md) |
...
| A reload keeps the session because the token crosses it; no token is asked for at page load | [tokens.md](tokens.md) |
| Signing out is two operations: revoke at Google, and remove this account's saved copy | [tokens.md](tokens.md) |
