...
`FakePlayer` throws the same `TypeError` before ready, because a fake that
answers straight away is *better* than YouTube, and every before-ready bug is
then green in the suite and a console error in a living room.
