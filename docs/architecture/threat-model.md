...
| T28 | `?at=` · **T** | Gets the viewer to open a crafted link | The channel runs at a different hour of its own schedule. The parameter is parsed to a millisecond offset, rejected unless it lands inside the broadcast day already in progress, and anything unparseable yields 0; it reaches a clock and nothing else. | CWE-20 | **Accept**: the value shifts a displayed time and reaches no sink beyond the clock | `offsetFromQuery` in [offsetClock.ts](../../src/clock/offsetClock.ts) |
...
| T34 | Sign-out · **R** | Is offline, or has the GIS script blocked, at the moment the viewer signs out | Half a sign-out. `signOut()` revokes first because `revoke` needs a live token, and drops the local token whatever comes back, so this page is signed out either way. Two residues are possible: a revocation that never reached Google leaves the grant standing on the account, and because `googleSession` awaits the revocation before `forget()`, a failure there leaves the cached pool in the store. The viewer is told: *Signed out here. This browser could not clear its saved programme list.* Consent is revocable from the Google account itself at any time. | CWE-613 | **Accept**: the grant is Google's to hold and the account page revokes it; the local residue is disclosed rather than hidden | `signOut` in [googleTokenProvider.ts](../../src/library/googleTokenProvider.ts) and [session.ts](../../src/library/session.ts), `onSignOut` in [Channel.tsx](../../src/ui/Channel.tsx) |

## The accepted risks, collected

Ten threats are accepted, each for a reason that is a property of the design
