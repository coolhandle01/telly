...
- **Data stores**: the IndexedDB pool
  ...
  keyed per account; the access token in the tab's `sessionStorage`,
  which is a credential; the published bundle on Pages, the CI environments
  `github-pages` and `commitlint`.
- **Data flows**: consent and token; the revocation that hands the grant back; API
...
|---|---|---|
| **B1** | browser origin ↔ `accounts.google.com` and `oauth2.googleapis.com` | the GIS script, the consent popup, the access token, and the revocation that hands the grant back, which the GIS script posts to `oauth2.googleapis.com/revoke` |
...
|---|---|---|---|---|---|---|
| T1 | Viewer identity · **S** | none | The app authenticates nobody. Identity is the Google session in the browser, and authorisation is a consent the viewer grants to a scope. The app learns one identifier: `ownerId()`, the signed-in account's own channel id from `channels.list` with `mine=true`, which keys the cache. | CWE-287 | **Transfer**: to Google Identity Services | `GoogleTokenProvider` in [googleTokenProvider.ts](../../src/library/googleTokenProvider.ts), `YouTubePoolSource.ownerId` in [youTubePoolSource.ts](../../src/library/youTubePoolSource.ts) |
...
| T36 | Two accounts, one page · **I** · **T** | Signs in as a second account on a page where the first account's session ended without a sign-out | `ownerId()` memoises the owner id. Kept past a session that ended because the token expired or YouTube refused it, it would file a second account that then signs in on the same page, without a reload, under the first account's key, and the first account would be shown the second account's pool if it signed in again while that record was fresh. Reproduced on 26 September 2026 by driving `createPoolSource` through a token change without a sign-out. `createPoolSource` subscribes to the token provider and calls `forget()` whenever the session ends, so the owner id goes with the session however it ends. `CachedPoolSource` keeps no key of its own past that: a load settles its key from the owner id before it fetches, and a sign-out settles it the same way, so neither a load that lands late nor the next account's sign-out reaches the first account's record. | CWE-488 | **Mitigate** | `ownerId` and `forget` in [youTubePoolSource.ts](../../src/library/youTubePoolSource.ts), the `subscribe` call in [createPoolSource.ts](../../src/library/createPoolSource.ts), `load` and `forget` in [cachedPoolSource.ts](../../src/library/cachedPoolSource.ts) |
