...
yesterday's card. Adding a design changes which card falls on which day. That is
fine, nobody has a right to Tuesday's card, and it is exactly why the rotation
...
(`Sound.prepare()`, run when the set is switched on), much as Google's script is
fetched on mount rather than on the sign-in click, so that a gesture the browser
requires is not lost to a wait.
